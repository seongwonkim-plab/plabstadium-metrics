...
- **plab.ts 캐시 전략 개편: Vercel Data Cache + 쿼리 타임아웃**
  ...
  - 개별 쿼리 15초 타임아웃 (`AbortController`) → API 무응답 시 빠른 실패
  - success=false 응답과 예외는 캐시 안 함 → 다음 호출에서 재시도 가능
  - 효과: API 다운 중에도 마지막 성공 데이터 30분간 유지되어 페이지가 즉시 렌더
- **12개월 트렌드 그래프 임시 비활성 (API 부하 완화)**
  - Plab API 게이트웨이가 우리 프로젝트만 반복 503 → 12개월치 배치 조회조차 무거운 것으로 추정
  - 월간/지점 페이지 데이터 조회를 **선택 월 (+ 이전 월) 만** 으로 축소
  - 페이지당 쿼리: 7개 → **~3~4개**
  - TrendChart 자리에 노란 배너로 "임시 비활성" 안내 노출
