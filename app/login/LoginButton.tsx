...
export function LoginButton({ callbackUrl }: { callbackUrl: string }) {
  ...
  useEffect(() => {
    // /api/auth/csrf 호출 → 응답 헤더의 Set-Cookie 로 브라우저에 쿠키 저장
    // 응답 JSON 의 token 을 hidden 필드로 사용 → 서버에서 폼 값과 쿠키가 매칭됨
    let cancelled = false
    fetch("/api/auth/csrf", { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((d: { csrfToken?: string }) => {
        if (!cancelled) setCsrfToken(d.csrfToken ?? null)
      })
      .catch((e) => {
        if (!cancelled) setError(e instanceof Error ? e.message : "csrf 조회 실패")
      })
    return () => {
      cancelled = true
    }
